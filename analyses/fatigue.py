...
def defdyn(R, F, LD, LR, ma, mr, mf, activ, T):
    if ma < T:
        if mr > T - ma:
            c = LD * (T - ma)
        else:
...
def dyn(t, X):
    ...
    rc_FR = FR_Percent * (ma_FR + mr_FR)
    rc_FF = FF_Percent * (ma_FF + mr_FF)
    ...
    (madot_S, mrdot_S, mfdot_S) = defdyn(R_S, F_S, LD_S, LR_S, ma_S, mr_S, mf_S, activ_S, TL/S_Percent)
    (madot_FR, mrdot_FR, mfdot_FR) = defdyn(R_FR, F_FR, LD_FR, LR_FR, ma_FR, mr_FR, mf_FR, activ_FR, TL/FR_Percent-ma_S)
    (madot_FF, mrdot_FF, mfdot_FF) = defdyn(R_FF, F_FF, LD_FF, LR_FF, ma_FF, mr_FF, mf_FF, activ_FF, TL/FF_Percent-ma_S-ma_FR)
